...
# ******************************************************
# Explore the clades excluded by the 5% samples filter
# ******************************************************
length(excluded_from_met_corr_filt)
wSE = grep("^S_", excluded_from_met_corr_filt)
species_ex = excluded_from_met_corr_filt[wSE]
length(species_ex)
species_ex

wGE = grep("^G_", excluded_from_met_corr_filt)
genus_ex = excluded_from_met_corr_filt[wGE]
length(genus_ex)
genus_ex

wFE = grep("^F_", excluded_from_met_corr_filt)
family_ex = excluded_from_met_corr_filt[wFE]
length(family_ex)
family_ex

wOE = grep("^O_", excluded_from_met_corr_filt)
order_ex = excluded_from_met_corr_filt[wOE]
length(order_ex)
order_ex

wCE = grep("^C_", excluded_from_met_corr_filt)
class_ex = excluded_from_met_corr_filt[wCE]
length(class_ex)
class_ex

wPE = grep("^P_", excluded_from_met_corr_filt)
phylum_ex = excluded_from_met_corr_filt[wPE]
length(phylum_ex)
phylum_ex

# what are the abundances of these clades
dim(met_corr_filt_clades)
abundance_of_excluded_phylum = met_corr_filt_clades [,phylum_ex]
mean_abundance_of_excluded_phylum = mean (abundance_of_excluded_phylum)
boxplot(abundance_of_excluded_phylum, xlab = ("Ascomycota"), ylab = "relative abundance")
wpres = abundance_of_excluded_phylum > 0
sum(wpres)
abundance_of_excluded_phylum[wpres]
# present in 15 samples

abundance_of_excluded_orders = met_corr_filt_clades [,order_ex]
dim (abundance_of_excluded_orders)
head(abundance_of_excluded_orders)
abundance_of_excluded_orders = met_corr_filt_clades [,order_ex]
boxplot(abundance_of_excluded_orders, xlab = ("Excluded orders"), ylab = ("abundance"))
# order 1 
boxplot(abundance_of_excluded_orders[,1], xlab = ("Chromatiales"), ylab = ("abundance"))
wo1p = (abundance_of_excluded_orders[,1]) > 0
sum(wo1p)
abundance_of_excluded_orders[wo1p,]
# order 2
boxplot(abundance_of_excluded_orders[,2],  xlab = ("Saccharomycetales"), ylab = ("abundance"))
wo2p = (abundance_of_excluded_orders[,2]) > 0
sum(wo2p)
abundance_of_excluded_orders[wo2p,]

abundance_of_excluded_families = met_corr_filt_clades [,family_ex]
dim (abundance_of_excluded_families)
head(abundance_of_excluded_families)
abundance_of_excluded_families = met_corr_filt_clades [,family_ex]
boxplot(abundance_of_excluded_families, xlab = ("Excluded families"), ylab = ("abundance"))

abundance_of_excluded_genera = met_corr_filt_clades [,genus_ex]
dim (abundance_of_excluded_genera)
head(abundance_of_excluded_genera)
abundance_of_excluded_genera = met_corr_filt_clades [,genus_ex]
boxplot(abundance_of_excluded_genera, pch = ".", cex = 3.5, text(seq(1, 10, by=1), par("usr")[3] - 0.2, srt = 45, pos = 1, xpd = TRUE), xlab = ("Excluded genera"), ylab = ("abundance"))

abundance_of_excluded_species = met_corr_filt_clades [,species_ex]
dim (abundance_of_excluded_species)
head(abundance_of_excluded_species)
abundance_of_excluded_species = met_corr_filt_clades [,species_ex]
plot(abundance_of_excluded_species[,1], xlab = ("samples"))
plot(abundance_of_excluded_species[,2], xlab = ("samples"))
plot(abundance_of_excluded_species[,17], xlab = ("samples"))
boxplot(abundance_of_excluded_species, xlab = ("Excluded species"), ylab = ("abundance"))
#•••••••••••••••••••••••••••
wSpec = grep("^S_", met_corr_sample_filt_clades)
species_5p = met_corr_sample_filt_clades[wSpec]
length(species_5p)
species_5p

wGen = grep("^G_", met_corr_sample_filt_clades)
genus_5p = met_corr_sample_filt_clades[wGen]
length(genus_5p)
genus_5p

wFam = grep("^F_", met_corr_sample_filt_clades)
family_5p = met_corr_sample_filt_clades[wFam]
length(family_5p)
family_5p

wOrd = grep("^O_", met_corr_sample_filt_clades)
order_5p = met_corr_sample_filt_clades[wOrd]
length(order_5p)
order_5p

wClas = grep("^C_", met_corr_sample_filt_clades)
class_5p = met_corr_sample_filt_clades[wClas]
length(class_5p)
class_5p

wPhy = grep("^P_", met_corr_sample_filt_clades)
phylum_5p = met_corr_sample_filt_clades[wPhy]
length(phylum_5p)
phylum_5p

## What were the clade numbers before sample filtering?
wSpecCF = grep("^S_", (colnames(met_corr_filt_clades)))
species_cf = met_corr_filt_clades[,wSpecCF]
length(colnames(species_cf))
head(colnames(species_cf))

wGenCF = grep("^G_", (colnames(met_corr_filt_clades)))
genus_cf = met_corr_filt_clades[,wGenCF]
length(colnames(genus_cf))
head(colnames(genus_cf))

wFamCF = grep("^F_",(colnames(met_corr_filt_clades)))
family_cf = met_corr_filt_clades[,wFamCF]
length(colnames(family_cf))
head(colnames(family_cf))

wOrdCF = grep("^O_", (colnames(met_corr_filt_clades)))
order_cf = met_corr_filt_clades[,wOrdCF]
length(colnames(order_cf))
head(colnames(order_cf))

wClasCF = grep("^C_", (colnames(met_corr_filt_clades)))
class_cf = met_corr_filt_clades[,wClasCF]
length(colnames(class_cf))
head(colnames(class_cf))

wPhyCF = grep("^P_", (colnames(met_corr_filt_clades)))
phylum_cf = met_corr_filt_clades[,wPhyCF]
length(colnames(phylum_cf))
head(colnames(phylum_cf))
